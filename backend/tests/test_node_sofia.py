...
class TestSofiaNode:
    ...
    def test_sofia_node_has_llm(self):
        """Test that SofiaNode has LLM configured."""
    ...
    async def test_sofia_evaluate_returns_correct_keys(self):
        ...
        mock_result.dict.return_value = {
            "score": 88,
            "notes": "Promising vintage with great potential",
            "confidence": 0.87,
            "techniques_used": ["innovation_scan", "tech_analysis"],
            "aspects": {"innovation": 90, "modernity": 85},
        }

        with patch.object(SofiaNode, "get_prompt") as mock_get_prompt:
...
class TestSofiaPrompt:
    ...
    def test_sofia_prompt_contains_vineyard_scout_theme(self):
        ...
        with patch("app.graph.nodes.base.ChatGoogleGenerativeAI"):
            ...
            # Verify system message contains vineyard scout references
            system_message = str(prompt_messages[0])
            assert (
                "Vineyard Scout" in system_message or "scout" in system_message.lower()
            )
    ...
    def test_sofia_prompt_focuses_on_innovation(self):
        ...
        with patch("app.graph.nodes.base.ChatGoogleGenerativeAI"):
            ...
            system_message = str(prompt_messages[0])
            assert (
                "innovation" in system_message.lower()
